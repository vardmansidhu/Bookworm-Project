  ...
  useEffect(() => {
    if (cookies.user) {
      if (selectedLanguage == null) {
        fetch(
          `http://localhost:8080/api/product/getByTypeNotInShelf/${typeId}/${cookies.user}`
        )
          .then((res) => res.json())
          .then((data) => setProducts(data));
      } else {
        fetch(
          `http://localhost:8080/api/product/getByLanguageAndTypeNotInShelf/${selectedLanguage}/${typeId}/${cookies.user}`
        )
          .then((res) => res.json())
          .then((data) => setProducts(data));
      }
  ...
  return (
    <>
      ...
      >
        {products
          .filter((product) =>
            selectedGenre ? product.genreId.genreId === selectedGenre : product
          )
          .map((product) => {
                ...
                <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                  ...
                  <div className="col p-4 d-flex flex-column position-static">
                    ...
                    <p
                      ...
                      className="text-primary icon-link gap-1 icon-link-hover"
                    ...
                    >
                      ...
                      >
                        ...
                        <Dropdown
                          style={{
                            display: product.rentable ? "inline" : "none",
                            marginLeft: "10px",
                          }}
                          onSelect={(key) =>
              ...
              <Container>
                ...
                <Row>
                  <Col>
                    ...
                    {selectedProduct.productEngName}
                  </Col>
                  <Col>
                    <strong>Price:</strong>{" "}
                    {selectedProduct.sprcialCost ?? selectedProduct.offerPrice}
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <strong>Description:</strong> {selectedProduct.longDesc}
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <strong>Publisher:</strong> {selectedProduct.publissher}
                  </Col>
                  <Col>
                    <strong>Genre:</strong> {selectedProduct.genreId.genreDesc}
                  </Col>
                </Row>
              </Container>
            </Modal.Body>
